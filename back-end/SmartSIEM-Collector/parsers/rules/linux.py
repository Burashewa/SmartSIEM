...
# Optimized Kali/systemd ISO8601:
# - fixed-width date/time tokens
# - optional fractional seconds with bounded precision
# - explicit timezone token
...
# pam_unix sudo auth failure metadata line (no passwd prompt)
...
# systemd Started / Finished "...service - ..."
_SYSTEMD_STARTED = re.compile(
    r"^Started\s+(?P<service>\S+)\s+-",
)

_SYSTEMD_FINISHED = re.compile(
    r"^Finished\s+(?P<service>\S+)\s+-",
)
...
LINUX_RULES: tuple[RegexRule, ...] = (
    ...
    # Keep sudo ahead of generic parent syslog flow via high message priority.
