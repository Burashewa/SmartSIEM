...
from app.normalizers.ocsf_model import OCSFEvent
...
class LogToOCSFMapper:
    ...
    def map(self) -> dict[str, Any]:
        ...
        category = self._pick_nested(self._event, "category") or ecs_action_meta["category"]
        severity = self._pick_nested(self._event, "severity") or ecs_action_meta["severity"]
        outcome = self._pick_nested(self._event, "outcome") or ecs_action_meta["outcome"]
        event_action = (
            self._pick_nested(self._event, "action")
            or self._pick_field("event_action", "ecs_action")
            or ecs_action_meta["event_action"]
        )
        timestamp = self._normalize_timestamp(self._pick_field("timestamp", "EventReceivedTime", "time"))
    ...
    def _ecs_action_and_meta(self) -> dict[str, Any]:
        ...
        if lt == "unix_chkpwd":
            return {"event_action": "password_check", "category": "authentication", "event_type": "event", "outcome": "failure", "severity": "medium", "command_line": None, "service_name": None}
        if lt == "pam_sudo_auth_failure":
            return {"event_action": "sudo_auth", "category": "authentication", "event_type": "event", "outcome": "failure", "severity": "high", "command_line": None, "service_name": None}
        if lt == "sudo_incorrect_password":
            return {"event_action": "sudo_command", "category": "authentication", "event_type": "event", "outcome": "failure", "severity": "medium", "command_line": command, "service_name": None}
        if lt == "sudo":
            return {"event_action": "sudo_command", "category": "authentication", "event_type": "event", "outcome": "success", "severity": "medium", "command_line": command, "service_name": None}
        if lt == "dhcp_lease":
            return {"event_action": "dhcp_lease", "category": "network_activity", "event_type": "event", "outcome": "success", "severity": "low", "command_line": None, "service_name": None}
        if lt == "systemd_started":
            return {"event_action": "service_started", "category": "host", "event_type": "start", "outcome": "success", "severity": "low", "command_line": None, "service_name": service}
        if lt == "systemd_finished":
            return {"event_action": "service_finished", "category": "host", "event_type": "end", "outcome": "success", "severity": "low", "command_line": None, "service_name": service}
        return {
            "event_action": lt,
    ...
    def _determine_category(self) -> str:
        ...
        if self._log_type in self._NETWORK_TYPES or any(token in text_blob for token in ("network", "dns", "ip", "port", "connection")):
...
def normalize(log_type: str, fields: dict[str, Any], raw: str | dict[str, Any], source: str = "") -> OCSFEvent:
    _ = source
